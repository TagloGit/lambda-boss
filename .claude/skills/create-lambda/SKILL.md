...
- **For per-rank unranking algorithms, carry state via HSTACK row + REDUCE/VSTACK, not via SetVars/GetVar inside MAP.** When generating C(n,k)-style results where each rank produces a row of indices, the natural shape `MAP(SEQUENCE(total), LAMBDA(rank, GetVar(unrank(rank), "acc")))` followed by `TEXTSPLIT(TEXTJOIN("#", FALSE, strs), "|", "#")` reshape works for `total=1` but returned a scalar (formula error) for `total>1` in our environment — single-rank tests passed, multi-rank tests failed with NRE in the harness's array path. Prefer: have `unrank(rank)` return a 1×k row (state grown via HSTACK across REDUCE slots), then aggregate ranks with `IF(total=1, unrank(1), REDUCE(unrank(1), SEQUENCE(total-1,1,2), LAMBDA(acc, rank, VSTACK(acc, unrank(rank)))))`. The HSTACK/VSTACK pattern keeps everything as native arrays end-to-end and avoids the text-encoded round-trip.
