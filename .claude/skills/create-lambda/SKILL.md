...
Keep the scalar body inside an inline `scalar` LAMBDA within the `LET`, then dispatch with an **unconditional MAP** — MAP hands its callback a true scalar for every element and accepts a plain scalar input, so no shape branch is needed:
...
//  Procedure
    scalar, LAMBDA(<paramName>,
              LET(... scalar body ...,
                  <scalar result>)),
    result, MAP(<liftParam>, scalar),
    IF(Help?, Help, result)
```

**Never branch `IF(OR(ROWS(x) > 1, COLUMNS(x) > 1), MAP(x, scalar), scalar(x))`.** A 1×1 *array* — produced by `CHOOSEROWS`, `TAKE`, a single-row `FILTER`, or an array-lifted `XLOOKUP` — passes that check and reaches the scalar body, where shape-sensitive functions (`SEQUENCE(n)`, `REGEXEXTRACT(..., 1)`, `TEXTSPLIT`) lift element-wise over the 1×1 array and collapse to a single element. The result is silently wrong values, not an error (#318: CELLTOPOS parsed `"BY85"` as column AZ because `SEQUENCE({2})` is `{1}`).

The only time a shape branch is still needed is when the scalar path must return something MAP cannot produce per element — a whole row, a spilled array, or a range reference (TLOOKUP row mode, INCEL). Keep the branch there, but deref the scalar path's argument first: `scalar(@<liftParam>)` — `@` collapses a 1×1 array to its value and is a no-op on true scalars.
...
- **Test YAML: omit a *middle* optional arg with a bare `"="`.** The harness joins args with commas into `=NAME(arg1,arg2,...)` and `FormatArg` maps a leading-`=` string to `s[1..]` — so `"="` becomes the empty string, producing a bare comma (an omitted positional arg) rather than a value. To test e.g. `=SEARCHALL("aaaa","aa",,FALSE)` (skip `match_case`, set `overlapping`), write `args: ["aaaa", "aa", "=", "=FALSE"]`. Excel sees the gap as `ISOMITTED` and applies the default. This is the only way to reach a later optional param while defaulting an earlier one.
